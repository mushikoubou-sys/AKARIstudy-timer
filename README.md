# MIKABU Study Timer

小学生が毎日の学習とピアノ練習を楽しく記録できる、スマートフォン優先の学習タイマーです。

## 主な機能

- 7科目それぞれのスタート・一時停止・再開・完了・完了取り消し
- 同時タイマー防止と、再読み込み後の計測復元
- 日別の学習時間・完了数・ポイントをブラウザに保存
- 1週間／科目別グラフ、日別記録、ごほうび交換
- 保護者向けのお名前・ごほうび設定
- レスポンシブ表示、PWA／オフライン対応
- Firebase Authentication と Cloud Firestore による親子リアルタイム同期
- 娘モード／閲覧専用の保護者モード
- 既存の端末内記録を初回ログイン時にクラウドへ移行

## クラウド同期

ログイン不要の共有モードです。URLを開いた端末は同じ公開データを参照し、端末モードだけを端末ごとに保持します。Firestoreへは操作時のみ、累積時間・開始時刻・状態を保存し、保護者画面は2秒間隔で更新を取得します。実行中の秒数は各端末で計算します。URLを知る第三者も閲覧・変更できる構成である点に注意してください。

Firestoreのルールは [firestore.rules](./firestore.rules) と同じ内容をFirebase Consoleへ設定してください。Firebase Web設定値は公開クライアント識別子であり、管理者秘密鍵ではありません。データ保護はFirebase AuthenticationとFirestore Security Rulesで行います。

## 公開方法

静的ファイルだけで動作します。GitHub の Settings → Pages で、公開元を `main` ブランチのルートに設定してください。
